...
/**
 * Permit application data export - Student Blue Badge.
 * 
...
try {
    ...
    $dataExportFile = "ACST_IS_" . date('Ymd') . ".csv";
    ...
    try {
        ...
        $permitApplications = $parkingApplication->getPermitApplicationsByStatus("AP", "ACST");
        ...
        $permitApplications2 = $parkingApplication->getPermitApplicationsByStatus("RP", "ACST");
        
        // Check if there are any matching permits.
        if (count($permitApplications2) > 0) {
            // Get all of the matching permits ...
            foreach ($permitApplications2 as $permitApplication) {
                // ... but only if the print exclusion flag is set to "No".
                if ($permitApplication['print_exclude'] == 'n') {
                    // Update data export flag.
                    $dataInExport = TRUE;
    ...
    try {
        ...
        if ((count($permitApplications) > 0 || count($permitApplications2) > 0) && $dataInExport == TRUE) {
